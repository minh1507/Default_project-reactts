...
import { CDialog, CInput,CNotification } from '../../../components';
...
const Login = (props: Props) => {  
    ...
    return (
        <>        
            ...
            <CDialog title={"Gửi email khôi phục mật khẩu"} dialogVisible={dialogVisible_RestorePassword} onCancel={() => setDialogVisible_RestorePassword(false)}>
                <div className="row g-3 needs-validation form-register">
                    <div className="col-12">                      
                        <CInput placeholder='Email' value={Email_RestorePassword} onChange={(e:any) => {setEmail_RestorePassword(e)}} />                        
                    </div>
                    <div className="col-12">                            
                        <CButton title='Đồng ý' onClick={SendEmail} isFullWidth={true} />
                    </div>                    
                </div>                   
            </CDialog>
            <CDialog title={"Đăng ký tài khoản"} dialogVisible={dialogVisible_SocialLogin} onCancel={() => setDialogVisible_SocialLogin(false)}>
                <div className="row g-3 needs-validation form-register">
                    <div className="col-12">                      
                        <CInput disabled={true} placeholder='Email' value={InputSignupSocial.Email} onChange={(e:any) => {onChangeFormSignupSocial('Email', e)}} />
                        <label style={{color:"#1B74E4", fontSize: "0.9em"}}><b>Email đã được xác thực bởi {InputSignupSocial.Type}</b></label>
                    </div>
                    <div className="col-12">                      
                        <CInput placeholder='Họ và tên' value={InputSignupSocial.FullName} onChange={(e:any) => {onChangeFormSignupSocial('FullName', e)}} /> 
                    </div>
                    <div className="col-12">                      
                        <CInput placeholder='Tên đăng nhập' value={InputSignupSocial.UserName} onChange={(e:any) => {onChangeFormSignupSocial('UserName', e)}} /> 
                    </div>
                    <div className="col-12">                      
                        <CInput placeholder='Mật khẩu' type='password' value={InputSignupSocial.Password} onChange={(e:any) => {onChangeFormSignupSocial('Password', e)}} /> 
                    </div>                  
                    <div className="col-12">                            
                ...
                <div className="row justify-content-center">
                    ...
                    <div className="card mb-3">
                        ...
                        <div className="row g-3 needs-validation form-login">
                            <div className="col-12">                            
                                <CInput placeholder='Tên đăng nhập hoặc email' value={InputLogin.UserName} onChange={(e:any) => {onChangeFormLogin('UserName', e)}} /> 
                            </div>
                            <div className="col-12">                            
                                <CInput type='password' key="password1" placeholder='Mật khẩu' value={InputLogin.PassWord} onChange={(e:any) => {onChangeFormLogin('PassWord', e)}} /> 
                            </div>
                            <div className="col-12">                            
                                <CButton title='Đăng nhập' onClick={Login} isFullWidth={true} />
                            </div>
                            ...
                            <p className="small mb-0">Bạn chưa có tài khoản? 
                                <a className="link" onClick={() => { history.push('/signup'); }}>&ensp;Đăng ký tài khoản</a>
                                &ensp;*&ensp;
                                <a className="link" onClick={() => { setDialogVisible_RestorePassword(true) }}>Khôi phục mật khẩu</a>
                            </p>
                            </div>
                        </div>
                        </div>
