...
const QuanLyNhomSuKien = (props: Props) => {
  const [state, dispatch] = useReducer(Reducer, InitState);
  const SuKienId_Tree = useRef(Guid.Empty);
  const [SuKienId_List, setTreeSuKienId_List] = useState("");
  ...
  useEffect(() => {
    ...
    Actions.GetItems(SuKienId_Tree.current, dispatch);
  ...
  const DialogMemo = useMemo(() => {
    return (
      <>
        {dialogVisible == true ? (
          <CDialog
            title={SuKienId_List ? "Sửa nhóm sự kiện" : "Tạo mới nhóm sự kiện"}
            dialogVisible={dialogVisible}
            onCancel={() => setDialogVisible(false)}
          >
            <SuKienForm
              Id={SuKienId_List}
              TreeId={SuKienId_Tree.current}
...
export default connect(mapState, mapDispatchToProps)(QuanLyNhomSuKien);
